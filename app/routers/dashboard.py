...
def market_page(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(
        request,
        "market.html",
        {"news": news, "econ_events": econ_events, "earnings_events": earnings_events},
    )


@router.get("/ativo/{symbol}")
def asset_detail(request: Request, symbol: str):
    return templates.TemplateResponse(request, "asset.html", {"symbol": symbol.upper()})
